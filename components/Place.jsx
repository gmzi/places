...
export default function Place({ id, imageUrl, name, rating, reviews, categories, price, distance, location, address, transactions, phone, url, alias }) {
    const division = distance / 1609
    const miles = (Math.round(division * 100) / 100).toFixed(1)

    const [images, setImages] = useState()

    async function getPlacePics(placeId) {
        const call = await axios.get(`${BASE_URL}/api/details?id=${placeId}`)
        const newData = call.data.photos;
        setImages(newData)
    }
    ...
    return (
        <div className={homeStyles.Place}>
            {images ? (
                <ImageDisplay data={images} closeImages={closeImages} />
            ) : (
                <div className={homeStyles.placeDetails}>
                    <h4>{name}</h4>
                    <div>
                        <img className={homeStyles.ratingImg} src={`/stars/small_${ratingDisplay(rating)}.png`} alt="rating" />
                        <span className={homeStyles.ratingCount}>({reviews})</span>
                    </div>
                    <div>
                        <span className={homeStyles.placeDescription}>
                            {categories.map((cat, i) => categories[i + 1] ? `${cat.alias} - ` : `${cat.alias}`)}
                        </span>
                        <div className={homeStyles.priceAndDistance}>
                            <p>{price}</p>
                            <p> &#8226; </p>
                            <p>{miles} miles </p>
                        </div>
                        <a className={homeStyles.placeAddress} href={`http://maps.google.com/maps?q=${address}`} rel="noreferrer" target="_blank">{location}</a>
                        <div className={homeStyles.placeTransactions}>
                            ...
                            (
                                <p key={`${id}-${trans}-span`}>
                                    <svg
                                        ...
                                        <path fill="green" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" />
                                    </svg>
                                    <span>{trans}</span>
                                </p>
                            ))}
                    ...
                    <div className={homeStyles.bottomRow}>
                        {
                            imageUrl ? (
                        ...
                        <div className={homeStyles.linksContainer}>
                            <Link href={`https://www.yelp.com/menu/${alias}`}>
                                <a className={homeStyles.placeYelp} target="_blank">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="15"
                                        height="15"
                                        ...
                                        <path d="M75,17.22v0a1.51,1.51,0,0,0-.09-.28l-.05-.1a1.5,1.5,0,0,0-.08-.15l0-.06a1.48,1.48,0,0,0-.42-.38l-.21-.11h0l-33-13.5A1.5,1.5,0,0,0,39,4V16H27.5A1.5,1.5,0,0,0,26,17.5v75A1.5,1.5,0,0,0,27.5,94h46A1.5,1.5,0,0,0,75,92.5v-75h0A1.49,1.49,0,0,0,75,17.22Zm-33-11L65.87,16H42ZM72,91H29V19H72Z" />
                                        <path d="M38.92,34.5A11.76,11.76,0,0,0,49,46.12V53H44a1.5,1.5,0,0,0,0,3H57a1.5,1.5,0,0,0,0-3H52V46.17A11.76,11.76,0,0,0,62.42,34.5a1.5,1.5,0,0,0-1.5-1.5H40.42A1.5,1.5,0,0,0,38.92,34.5ZM59.3,36a8.75,8.75,0,0,1-17.24,0Z" />
                                        <path d="M36,68.5A1.5,1.5,0,0,0,37.5,70h26a1.5,1.5,0,0,0,0-3h-26A1.5,1.5,0,0,0,36,68.5Z" />
                                        <path d="M57.17,75h-13a1.5,1.5,0,0,0,0,3h13a1.5,1.5,0,0,0,0-3Z" />
                                    </svg> <span className={homeStyles.btnText}>Menu</span>
                                </a>
                            </Link>
                            ...
                            <Link href={url}>
                                <a className={homeStyles.placeYelp} target="_blank">
                                    ...
                                    </svg> <span className={homeStyles.btnText}>Yelp</span>
                                </a>
                            </Link>

                            <a className={homeStyles.placePhone} href={`tel:${phone}`}>
                                ...
                                </svg> <span className={homeStyles.btnText}>Call</span>
                            </a>

                            <Link href={`http://maps.google.com/maps?q=${address}`}>
                                <a className={homeStyles.placeYelp} target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        width="15"
                                        height="15"
                                    ...
                                    </svg> <span className={homeStyles.btnText}>Go</span>
                                </a>
                            </Link>
                        </div>
                    </div>
